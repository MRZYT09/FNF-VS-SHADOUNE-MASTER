...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		else if (SONG.song.toLowerCase() == 'thorns')
		{
		...
		else if (SONG.song.toLowerCase() == 'aquinoby')
		   {
			defaultCamZoom = 0.9;
			curStage = 'webadas';
		...
		else if (SONG.song.toLowerCase() == 'uhc')
		   {
			defaultCamZoom = 0.90;
			curStage = 'uhc';
		...
		else if (SONG.song.toLowerCase() == 'you-are-mine')
			{
			 defaultCamZoom = 0.90;
		...
		else if (SONG.song.toLowerCase() == 'shadoune')
			{
			defaultCamZoom = 0.90;
			curStage = 'shadoune';
		 ...
		 else if (SONG.song.toLowerCase() == 'decition')
		{
		 ...
		 else if (SONG.song.toLowerCase() == 'shadpurgation')
		{
			defaultCamZoom = 0.90;
			curStage = 'shadpurgation';
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 3 ? "- UHC" : storyDifficulty == 2 ? "- Hard" : storyDifficulty == 1 ? "- Normal" : "- Easy") + "- kade engine -" + MainMenuState.kadeEngineVer + " - VS SHADOUNE -" + (FlxG.save.data.etternaMode ? "E.Mode" : "FNF"), 16);
		...
		scoreTxt.setFormat(Paths.font("minecraftia.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
						...
						new FlxTimer().start(0.2, function(swagTimer:FlxTimer)
						{
							...
							else
							{
								new FlxTimer().start(1, function(swagTimer:FlxTimer)
								{
									...
									{
										FlxG.sound.play(Paths.sound('uhc sound', 'shared'));
										startCountdown();
	...
	private function generateSong(dataPath:String):Void
	{
		...
		//pls remaster this
		/*var beats = 600* (100/songData.bpm);


		if(SONG.song.toLowerCase()== 'uhc' && dad.curCharacter == 'shadoune')
		{
			for (x in 0...evento.length)
			{
				var daleomueresTimer = evento[x];
				var daleomueresNota:Note = new Note(daleomueresTimer * beats, 1, null, false, true);
				daleOmueresNota.scrollFactor.set ();
				unspawnNotes.push(daleOmueresNota);
				daleomueresNota.mustPress = true;
				daleomueresNota.x += FlxG.width / 2;
			}
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (SONG.song.toLowerCase() == 'shadoune' || SONG.song.toLowerCase() == 'uhc' || SONG.song.toLowerCase() == 'decition' || SONG.song.toLowerCase() == 'you-are-mine')
				{
	...
	{/*
		...
		{
            var tween = FlxTween.tween(noteCam, {x: noteCam.x + 20}, 0.5, {
			type: PINGPONG
			});
		}
		*/
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat == 530 && SONG.song.toLowerCase() == 'shadpurgation')
			{
				dad.playAnim('SWORD', true);
		...
		if (curBeat == 20 && SONG.song.toLowerCase() == 'uhc')
			{
				var tween = FlxTween.tween(noteCam, {x: noteCam.x + 100}, 0.1, {
				type: PINGPONG
				});
				var tween = FlxTween.tween(noteCam, {y: noteCam.y + 20}, 0.1, {
				type: PINGPONG
				});
			}

			if (curBeat == 16 && SONG.song.toLowerCase() == 'shadpurgation')
				{
					var tween = FlxTween.tween(noteCam, {x: noteCam.x + 100}, 0.1, {
					type: PINGPONG
					});
